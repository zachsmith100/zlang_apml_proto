...
std::string CUniVar::ToString(const int width, const int precision)const
{
	std::string str;

	if ((GetType() == UV_INT) || (GetType() == UV_DOUBLE) || (GetType() == UV_ARB_DOUBLE))
	{
		str = Number::RefinePrecision(this, precision).ToString();
	}
	else
	{
		str = ToString();		
...
std::string CUniVar::ToString()const
{
	if (m_type == UV_INT)
	{		
		return IntToStr((int)_INT(m_value));
	}
	else if (m_type == UV_DOUBLE)
	{		
#ifdef WIN32
