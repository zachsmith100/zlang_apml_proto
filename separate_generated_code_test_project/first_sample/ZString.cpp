...
CUniVar String::Format(int count, ...)
{
	va_list vl; 
	va_start(vl, count); 	
	CUniVar* str_ = va_arg(vl, CUniVar*);
	CONVERT_TO_STRING_IF_NEEDED

	std::string str = str_->ToString();
	for (int i = 0; i < count - 1; i++)
	{
		CUniVar* param = va_arg(vl, CUniVar*);		
		std::string paramId = "{" + IntToStr(i) + "}";
		if (param->GetType() == UV_CLASS)
		{
			CClassDataContainer* cdt = param->ToClass();
			if ((cdt->IsPropertyExist("value")) && ((cdt->IsPropertyExist("precision")) || (cdt->IsPropertyExist("width"))))
			{
				CUniVar value = cdt->GetProperty("value");
				CUniVar precision_ = (cdt->IsPropertyExist("precision")) ? cdt->GetProperty("precision") : CUniVar(0);
				CUniVar width_ = (cdt->IsPropertyExist("width")) ? cdt->GetProperty("width") : CUniVar(0);		

				int width = width_.ToInt();
				int precision = precision_.ToInt();
				
				if ((width >= 0) && (precision >= 0))
				{					
					StringReplace(str, paramId, value->ToString(width, precision));
				}
				else if (width < 0)
				{
					Log.Warnings.AddLine2s("Attempt to use incorrect width of parameter for string format after line %s in %s", currentLine, getCurrentMethodName());
				}
				else if (precision < 0)
				{
					Log.Warnings.AddLine2s("Attempt to use incorrect precision of parameter for string format after line %s in %s", currentLine, getCurrentMethodName());
				}
			}
			else
			{
				Log.Warnings.AddLine2s("Attempt to use incorrect parameter for string format after line %s in %s", currentLine, getCurrentMethodName());
			}
		}
		else
		{			
			StringReplace(str, paramId, param->ToString());
		}
	}
	va_end(vl);	 
