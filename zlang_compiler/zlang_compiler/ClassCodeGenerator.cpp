...
DECLARE_HANDLER(CClassCodeGenerator::ProcessConstructor)
{
	...
	bool isInplaceConstructor = (context.objectName == "");
	if (isInplaceConstructor)
	{
		context.objectName = CUniqueIdGenerator::GetInstance()->GetId();
		CUnorderedVarDeclarations::GetInstance()->AddVariable(context.objectName);
		context.fullObjectName = context.objectName;
	}

	callConstructorTpl->SetVariableValue(initCodeIndex, GetCodeForNode(nd->center, context, true));	
	callConstructorTpl->SetVariableValue(objectNameIndex, context.objectName);
	callConstructorTpl->SetVariableValue(fullObjectNameIndex, context.fullObjectName);	
	...
	if (isInplaceConstructor)
	{
		CUnorderedVarDeclarations::GetInstance()->AddCode(callConstructorTpl->GetText());
		std::string resultObjectName = context.objectName;
		context.EndObject();
