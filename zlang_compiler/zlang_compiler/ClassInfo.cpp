...
void CClassInfo::AddMethodExt(const std::string& name, const unsigned int parametersCount, const unsigned int minParametersCount, const unsigned int line, const std::vector<MethodParameter>& parameters, const bool noDeclareMethod, const std::string& defaultResultField)
{
	std::vector<bool> isOutParameter;
	AddMethod(name, parametersCount, line, isOutParameter, parameters, noDeclareMethod, defaultResultField);
		
	for (unsigned int k = 0; k < minParametersCount; k++)
	{
		isOutParameter.push_back(false);
	}

	AddMethod(name, minParametersCount, line, isOutParameter, parameters, noDeclareMethod, defaultResultField);
}
...
void CClassInfo::AddMethod(const std::string& name, const unsigned int parametersCount, const unsigned int line, const std::vector<bool>& isOutParameter, const std::vector<MethodParameter>& parameters, const bool noDeclareMethod, const std::string& defaultResultField)
{
	...
	assert(parametersCount == isOutParameter.size() || parametersCount == 0xFFFFFFFF);
...
bool CClassInfo::IsVariableParameterMethod(const std::string& methodName)
{
	MethodMap::const_iterator it = m_methods.find(methodName);

	if (it != m_methods.end())
	if (((*it).second.parametersCount.size() == 2) && ((*it).second.parametersCount[0] == 0xFFFFFFFF))
	{
		return true;
	}	
...
MethodCallValidateError CClassInfo::ValidateMethodCall(const std::string& methodName, const unsigned int parametersCount, const bool isInnerCall)const
{
	...
	if (itCheck == (*it).second.parametersCount.end())
	{
		if (((*it).second.parametersCount.size() == 2) && ((*it).second.parametersCount[0] == 0xFFFFFFFF))
		{
			return (parametersCount >= (*it).second.parametersCount[1]) ? OK : INVALID_PARAMETERS_COUNT;
		}
		else
		{
