...
std::string CFunctionsCodeGenerator::ProcessMethodCallImpl(node* nd, CompileContext& context, const TypeOfMethodCall typeOfCall)
{
	...
	{
		classInfo = Classes::GetInstance()->GetClassByName(nd->left->token);
	}	
	...
	if ((nd->left) && (nd->center) && (nd->center->left) && (!nd->center->left->left))
	{
		...
		if ((classInfo) && (classInfo->GetType() == CLASS_TYPE_SINGLETON) && (classInfo->IsBuiltIn()))
		{
			...
			if (classInfo->IsVariableParameterMethod(methodName))
			{
				functionParams = IntToStr(parametersCount) + functionParams;
			}
			else
			{
				functionParams = (functionParams.size() == 0) ? "" : functionParams.substr(1, functionParams.size() - 1);
			}
			...
			standardObjectMethodCallTpl->SetVariableValue(methodParamsIndex, functionParams);
