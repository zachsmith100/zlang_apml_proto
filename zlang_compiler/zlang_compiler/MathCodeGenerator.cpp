...
{
	bool isFirstCmdInLine = IS_FIRST_COMMAND_IN_LINE;
	...
	{	
		...
		resultCode = classInstanceTpl->GetText();		
	}
	else
	{		
		...
		{	
			...
			context.fullObjectName = GetCodeForNode(nd->left, context);
			// now we will get only name of variable
			if (nd->left->token != NULL)
			{
				CHANGE_STATE(functionDeclaration, true);			
			}
			context.objectName = GetCodeForNode(nd->left, context);
			if (nd->left->token != NULL)
			{
				RESTORE_STATE(functionDeclaration);
			}
			std::string code = GetCodeForNode(nd->center, context);
			context.EndObject();
			resultCode = code + end;
		}
		else
		{
			...
			resultCode = changeValueTpl->GetText();				
		}		
	}

	if (isFirstCmdInLine)
	{
		resultCode = CUnorderedVarDeclarations::GetInstance()->GetDeclarationsString() + resultCode;
