...
PROCESS_TOKENS_START(CVariablesCodeGenerator)
	...
	PROCESS_TOKEN(OP_ARB_FLOAT_DECL, ProcessArbitraryFloatDeclaration);
	PROCESS_TOKEN(OP_ARB_FIXED_DECL, ProcessArbitraryFixedDeclaration);
	PROCESS_TOKEN(OP_INTEGER_DECL, ProcessIntegerDeclaration);
	PROCESS_TOKEN(OP_DOUBLE_DECL, ProcessDoubleDeclaration);
...
DECLARE_HANDLER(CVariablesCodeGenerator::ProcessIntegerDeclaration)
{
	DEFINE_END_SIGN(nd->line);
	...
	if (nd->left)
	{
		...
		if (nd->left->token)
		{
			defaultValueIsNumericConst = IsNumeric(nd->left->token);

			if (defaultValueIsNumericConst)
			if ((!IsSimpleHumeric(nd->left->token)) && (!IsHexHumeric(nd->left->token)))
			{
				CErrorReporter::GetInstance()->OnFatalError(WRONG_ATTEMPT_TO_INIT_INTEGER, nd->line, nd->left->token);
			}
		}		
	}
	...
	if (noDefaultValue)
	{
		USE_TPL(variables, declIntegerNoDefault);		
		USE_INDEX(declIntegerNoDefault, end);			
		declIntegerNoDefaultTpl->SetVariableValue(endIndex, end);	
		return declIntegerNoDefaultTpl->GetText();
	}
	else
	{
		if (defaultValueIsNumericConst)
		{
			USE_TPL(variables, declIntegerDefaultConst);			
			USE_INDEX(declIntegerDefaultConst, end);	
			USE_INDEX(declIntegerDefaultConst, value);			
			declIntegerDefaultConstTpl->SetVariableValue(valueIndex, nd->left->token);	
			declIntegerDefaultConstTpl->SetVariableValue(endIndex, end);	
			return declIntegerDefaultConstTpl->GetText();
		}
		else
		{
			USE_TPL(variables, declIntegerDefaultVar);			
			USE_INDEX(declIntegerDefaultVar, value);	
			USE_INDEX(declIntegerDefaultVar, end);				
			declIntegerDefaultVarTpl->SetVariableValue(valueIndex, GetCodeForNode(nd->left, context));	
			declIntegerDefaultVarTpl->SetVariableValue(endIndex, end);	
			return declIntegerDefaultVarTpl->GetText();
		}
	}
...
DECLARE_HANDLER(CVariablesCodeGenerator::ProcessDoubleDeclaration)
{
	DEFINE_END_SIGN(nd->line);
	...
	if (noDefaultValue)
	{
		USE_TPL(variables, declDoubleNoDefault);		
		USE_INDEX(declDoubleNoDefault, end);			
		declDoubleNoDefaultTpl->SetVariableValue(endIndex, end);	
		return declDoubleNoDefaultTpl->GetText();
	}
	else
	{
		if (defaultValueIsNumericConst)
		{
			USE_TPL(variables, declDoubleDefaultConst);			
			USE_INDEX(declDoubleDefaultConst, end);	
			USE_INDEX(declDoubleDefaultConst, value);			
			declDoubleDefaultConstTpl->SetVariableValue(valueIndex, nd->left->token);	
			declDoubleDefaultConstTpl->SetVariableValue(endIndex, end);	
			return declDoubleDefaultConstTpl->GetText();
		}
		else
		{
			USE_TPL(variables, declDoubleDefaultVar);			
			USE_INDEX(declDoubleDefaultVar, value);	
			USE_INDEX(declDoubleDefaultVar, end);				
			declDoubleDefaultVarTpl->SetVariableValue(valueIndex, GetCodeForNode(nd->left, context));	
			declDoubleDefaultVarTpl->SetVariableValue(endIndex, end);	
			return declDoubleDefaultVarTpl->GetText();
		}
	}
...
{		
	...
	res.push_back(OP_ARB_FLOAT_DECL);
	res.push_back(OP_ARB_FIXED_DECL);
	res.push_back(OP_DOUBLE_DECL);
	res.push_back(OP_INTEGER_DECL);
	return res;
}
