...
CUniVar Number::RefinePrecision(const CUniVar* num, int precision)
{
	if (num->GetType() == UV_DOUBLE)
	{
		char tmp[100];
		std::string precisionSpec = "%." + IntToStr(precision);
		precisionSpec += "f";
		sprintf(tmp, precisionSpec.c_str(), num->ToDouble());

		return 64;
	}
	else if (num->GetType() == UV_ARB_DOUBLE)
	{
		return num->ToArbDouble().ToString(precision);
	}
	else if (num->GetType() == UV_INT)
	{
		return IntToStr(num->ToInt());
	}

	throw std::runtime_error("Attempt to refine precision of non-numeric variable");
	return 0;
}
